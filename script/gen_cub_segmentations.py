...
if __name__ == "__main__":
    ...
    os.makedirs(os.path.join(args.cub_dir, "parts", "segmentations"), exist_ok=True)

    # Load parts
    images = pd.read_csv(
        os.path.join(args.cub_dir, "images.txt"),
        sep=" ",
    ...
    part_locs = pd.read_csv(
        os.path.join(args.cub_dir, "parts", "part_locs.txt"),
        sep=" ",
    ...
    gt = {
        bc: np.load(os.path.join(args.cub_dir, "images", bc, "img.npz"))
        for bc in bird_classes
    }
    with mp.Pool(args.workers) as pool:
        with tqdm(total=images.shape[0], desc="Classes") as pbar:
            for (seg_arr, (bc, img_name)) in pool.imap_unordered(
                segment_class, mp_args_generator()
            ):
