...
def create_tiled_image(imgs, gridheight, gridwidth, ds, imsize=112, gap=3):
    tiled = np.full(
        ((imsize + gap) * gridheight - gap, (imsize + gap) * gridwidth - gap, 3),
        255,
        dtype="uint8",
    )
    ...
    for x, img_dict in enumerate(imgs):
        ...
        if mark is not None:
            vis = ImageOps.expand(vis, border=10, fill=mark).resize(
                (imsize, imsize), resample=Image.BILINEAR
            )
        ...
        vis = np.array(vis)
        tiled[
            row * (imsize + gap) : row * (imsize + gap) + imsize,
            col * (imsize + gap) : col * (imsize + gap) + imsize,
            :,
        ] = vis
    return tiled
