...
def train_val_test_split(
    *args, val_size=0.1, test_size=0.1, random_state=None, **kwargs
):
    """
...
def load_cub(
    data_dir, random_state=None, max_classes=None, train_only=False, train_augment=True
):
    """
    Load CUB dataset.
    ...
    index = pd.read_csv(
        os.path.join(data_dir, "images.txt"), names=["image_id", "image_name"], sep=" "
    )
    ...
    attr_names = pd.read_csv(
        attr_names_fname, names=["attribute_id", "attribute_name"], sep=" "
    )
    attr_names["attribute_id"] = attr_names["attribute_id"] - 1
    # Replace ( ) for parsing later
    attr_names["attribute_name"] = (
        attr_names["attribute_name"].str.replace("(", "").str.replace(")", "")
    )
    attr_names["category"], attr_names["value"] = zip(
        *attr_names.attribute_name.str.split("::")
    )
    ...
    for image_id, image_name in tqdm(
        zip(index.image_id, index.image_name),
        desc="Loading CUB images",
        total=len(index.image_id),
    ):
...
class CUBDataset:
    def __init__(
        ...
        self.attr2cat = dict(
            zip(self.attr_metadata["attribute_id"], self.attr_metadata["category"])
        )
        self.cat2id = dict(
            (v, k) for k, v in enumerate(self.attr_metadata["category"].unique())
        )
...
class CUBSegmentationPrefetcher:
    ...
    def __init__(
        ...
        self.cub_loader = to_dataloader(
            self.cub, batch_size=batch_size, shuffle=randomize
        )
        self.indexes = range(0, len(self.cub))
        self.segmentations = {
            cn: np.load(
                os.path.join(
                    settings.DATA_DIRECTORY, "parts", "segmentations", f"{cn}.npz"
                )
            )
